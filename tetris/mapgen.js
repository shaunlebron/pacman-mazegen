...
var genRandom = function() {
  ...
  var gen = function() {
    ...
    var probStopGrowingAtSize = [
      // probability of stopping growth at sizes...
      0, // size 0
      0, // size 1
      0.1, // size 2
      0.5, // size 3
        ...
        while (size < 5) {
          ...
          if (size == 2) {
            ...
            if (
              ...
              c.connect[RIGHT] &&
              c.next[RIGHT] &&
              c.next[RIGHT].next[RIGHT]
            ) {
              if (
                longPieces < maxLongPieces &&
                Math.random() <= probExtendAtSize2
              ) {
          ...
          if (stop) {
            if (size == 1) {
              ...
              if (
                longPieces < maxLongPieces &&
                firstCell.x > 0 &&
                Math.random() <= probExtendAtSize3or4
              ) {
                var dirs = [];
                var dirsLength = 0;
                for (i = 0; i < 4; i++) {
  ...
  var cellIsCrossCenter = function(c) {
    return (
      c.connect[UP] && c.connect[RIGHT] && c.connect[DOWN] && c.connect[LEFT]
    );
  };
  ...
  var chooseNarrowCols = function() {
    var canShrinkWidth = function(x, y) {
      ...
      for (x0 = x; x0 < cols; x0++) {
        ...
        c2 = c.next[DOWN];
        if (
          (!c.connect[RIGHT] || cellIsCrossCenter(c)) &&
      ...
      for (; c2; c2 = c2.next[LEFT]) {
        ...
        if (
          (!c2.connect[LEFT] || cellIsCrossCenter(c2)) &&
  ...
  var chooseTallRows = function() {
    var canRaiseHeight = function(x, y) {
      ...
      for (y0 = y; y0 >= 0; y0--) {
        ...
        c2 = c.next[RIGHT];
        if (
          (!c.connect[UP] || cellIsCrossCenter(c)) &&
      ...
      for (; c2; c2 = c2.next[DOWN]) {
        ...
        // cannot proceed further without causing irreconcilable tight turns
        if (
          (!c2.connect[DOWN] || cellIsCrossCenter(c2)) &&
  ...
  var isDesirable = function() {
    ...
    var isVert = function(x, y) {
      ...
      if (x == cols - 1) {
        ...
        return (
          !q1[LEFT] && !q1[UP] && !q1[DOWN] && !q2[LEFT] && !q2[UP] && !q2[DOWN]
        );
      }
      return (
        !q1[LEFT] &&
  ...
  var joinWalls = function() {
    ...
    for (x = 0; x < cols; x++) {
      ...
      if (
        ...
        if (
          (!c.next[LEFT] || !c.next[LEFT].connect[UP]) &&
          ...
          if (
            !(
              c.next[DOWN] &&
              c.next[DOWN].connect[RIGHT] &&
              c.next[DOWN].next[RIGHT].connect[RIGHT]
            )
          ) {
    ...
    for (x = 0; x < cols; x++) {
      ...
      if (
        ...
        if (
          (!c.next[LEFT] || !c.next[LEFT].connect[DOWN]) &&
          ...
          if (
            !(
              c.next[UP] &&
              c.next[UP].connect[RIGHT] &&
              c.next[UP].next[RIGHT].connect[RIGHT]
            )
          ) {
            c.isJoinCandidate = true;
            if (Math.random() <= 0.25) {
...
var getTiles = function() {
  ...
  for (y = 0; y < subrows; y++) {
    for (x = 0; x < subcols; x++) {
      ...
      if (c) {
        // inside map
        if (
          (cl && c.group != cl.group) || // at vertical boundary
        ...
        // outside map
        if (
          (cl && (!cl.connect[RIGHT] || getTile(x - 1, y) == ".")) || // at right boundary
  ...
  */
  ...
  const emptyRow = "_".repeat(fullcols);
  const readableRows = [];
  for (let y = 0; y < subrows; y++) {
    const i = y * fullcols;
    readableRows.push(tiles.slice(i, i + fullcols).join(""));
  }
  const readableTiles = [
    emptyRow,
    emptyRow,
  ...
  const tileStr = emptyRow + emptyRow + tiles.join("") + emptyRow + emptyRow;
  return { readableTiles, tileStr, w: fullcols, h: subrows };
};
...
var drawCells = function(ctx, left, top, size, title, options) {
  ...
  for (i = 0; i < cols * rows; i++) {
    ...
    if (options.drawShrinkWidth && c.shrinkWidth) {
      ...
      ctx.save();
      ctx.translate(
        x * size + size - arrowsize - arrowsize,
        y * size + size / 2
      );
      ctx.moveTo(arrowsize, -arrowsize);
      ctx.lineTo(0, 0);
...
var mapgen = function() {
  ...
  const { tileStr, w, h, readableTiles } = getTiles();
  console.log(readableTiles);
  const map = new Map(w, h + 1, tileStr);
  map.name = "";
  map.wallFillColor = randomColor();
  map.wallStrokeColor = rgbString(
    hslToRgb(Math.random(), Math.random(), Math.random() * 0.4 + 0.6)
  );
